...
"[project]/node_modules/@21st-extension/toolbar/dist/utils-DRt8LwD0.js [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
__turbopack_context__.s({
    ...
    "n": ()=>h,
    "o": ()=>fetchSVGContent,
    "p": ()=>processSVGForInline,
    "q": ()=>q$1,
    "r": ()=>getElementAtPoint,
    "s": ()=>companionAnchorTagName,
    "u": ()=>usePlugins,
    "x": ()=>x$1,
    "y": ()=>y
});
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@21st-extension/toolbar/dist/jsx-runtime-BI3iOU20.js [app-client] (ecmascript)");
;
var t, r$1, u, i, o = 0, f = [], c = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"], e = c.__b, a = c.__r, v = c.diffed, l = c.__c, m = c.unmount, s = c.__;
function p(n, t2) {
...
const ConfigContext = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["K"])({
    config: void 0
});
function ConfigProvider(param) {
    ...
    return /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["u"])(ConfigContext.Provider, {
        value,
        children
    });
}
...
var generateId$1 = function() {
   ...
   */ register(methodHandlers) {
        Object.entries(methodHandlers).forEach((param)=>{
            let [methodName, handler] = param;
   ...
   */ callMethod(method, payload, onUpdate) {
        ...
        return new Promise((resolve, reject)=>{
            ...
            const timeout = setTimeout(()=>{
                this.pendingRequests.delete(id), reject(new Error("Request timed out: ".concat(method)));
            ...
            this.pendingRequests.set(id, {
                ...
                timeout,
                onUpdate
            }), (_a = this.ws) == null || _a.send(JSON.stringify(requestMessage));
        });
    }
    /**
   ...
   */ handleMessage(message) {
        ...
        switch(messageType){
            ...
            case "error":
                ...
                break;
            default:
                console.warn("Unknown message type: ".concat(messageType));
        }
    }
    /**
   ...
   */ async handleRequest(message) {
        ...
        if (!methodDef) {
            this.sendError(id, "Method not found: ".concat(method));
            return;
        }
        try {
   ...
   */ handleResponse(id, payload) {
        ...
        if (!pendingRequest) {
            console.warn("Received response for unknown request ID: ".concat(id));
            return;
        }
   ...
   */ handleUpdate(id, payload) {
        ...
        if (!pendingRequest || !pendingRequest.onUpdate) {
            console.warn("Received update for unknown request ID: ".concat(id));
            return;
        }
   ...
   */ handleError(id, error) {
        const pendingRequest = this.pendingRequests.get(id);
        if (!pendingRequest) {
            console.warn("Received error for unknown request ID: ".concat(id));
            return;
        }
   ...
   */ handleDisconnect() {
        if (this.isIntentionalClose) {
            console.log("WebSocket closed intentionally, not attempting to reconnect"), this.clearPendingRequests(new Error("Connection closed by user"));
            return;
        }
        this.reconnectAttempts < this.options.maxReconnectAttempts ? (this.reconnectAttempts++, console.log("Attempting to reconnect (".concat(this.reconnectAttempts, "/").concat(this.options.maxReconnectAttempts, ")...")), setTimeout(()=>this.reconnect(), this.options.reconnectDelay * this.reconnectAttempts)) : (console.error("Max reconnection attempts reached"), this.clearPendingRequests(new Error("Connection closed")));
    }
    /**
   * Clear all pending requests with an error
   * @param error Error to reject with
   */ clearPendingRequests(error) {
        this.pendingRequests.forEach((param)=>{
            let { reject } = param;
            reject(error);
   ...
   */ async close() {
    ...
    constructor(options = {}){
        this.ws = null, this.pendingRequests = /* @__PURE__ */ new Map(), this.reconnectAttempts = 0, this.methods = {}, this.isIntentionalClose = !1, this.options = {
            ...DEFAULT_OPTIONS,
            ...options
        };
    }
};
function validateWithZod(schema, data, context) {
    let silent = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : !1;
    const result = schema.safeParse(data);
    if (!result.success) {
        const error = new Error("Validation failed for ".concat(context, ": ").concat(result.error.message));
        if (silent) return console.error(error), data;
        throw error;
    }
    return result.data;
}
var ZodTypedBridge = class {
    async callMethod(method, request, options) {
        ...
        if (!methodContract) throw new Error("Method ".concat(String(method), " not found in contract"));
        const validatedRequest = validateWithZod(methodContract.request, request, "request for method ".concat(String(method))), onUpdate = options != null && options.onUpdate && methodContract.update ? (update)=>{
            var _a;
            if (methodContract.update) try {
                const validatedUpdate = validateWithZod(methodContract.update, update, "update for method ".concat(String(method)), !0);
                (_a = options.onUpdate) == null || _a.call(options, validatedUpdate);
            } catch (error) {
                console.error("Update validation failed:", error);
            }
        } : void 0, response = await this.bridge.callMethod(method, validatedRequest, onUpdate);
        return validateWithZod(methodContract.response, response, "response for method ".concat(String(method)));
    }
    register(implementations) {
        ...
        for (const [method, implementation] of Object.entries(implementations)){
            ...
            if (!methodContract) throw new Error("Method ".concat(method, " not found in contract"));
            wrappedImplementations[method] = async (request, sendUpdate)=>{
                const validatedRequest = validateWithZod(methodContract.request, request, "request for method ".concat(method)), wrappedSendUpdate = methodContract.update && sendUpdate ? (update)=>{
                    if (methodContract.update) try {
                        const validatedUpdate = validateWithZod(methodContract.update, update, "update for method ".concat(method), !0);
                ...
                return validateWithZod(methodContract.response, response, "response for method ".concat(method));
            };
        }
...
(function(util2) {
    ...
    util2.assertNever = assertNever, util2.arrayToEnum = (items)=>{
        ...
        for (const k2 of validKeys)filtered[k2] = obj[k2];
...
class ZodError extends Error {
        ...
        const mapper = _mapper || function(issue) {
            ...
            for (const issue of error.issues)if (issue.code === "invalid_union") issue.unionErrors.map(processError);
                ...
                for(; i2 < issue.path.length;){
                    ...
                    i2 === issue.path.length - 1 ? (curr[el] = curr[el] || {
                        _errors: []
    ...
    static assert(value) {
        if (!(value instanceof ZodError)) throw new Error("Not a ZodError: ".concat(value));
    }
    toString() {
        return this.message;
    }
    ...
    constructor(issues){
        var _this;
        super(), _this = this, this.issues = [], this.addIssue = (sub)=>{
            this.issues = [
                ...this.issues,
                sub
            ];
...
const errorMap = (issue, _ctx)=>{
    ...
    switch(issue.code){
        case ZodIssueCode.invalid_type:
            issue.received === ZodParsedType.undefined ? message = "Required" : message = "Expected ".concat(issue.expected, ", received ").concat(issue.received);
            break;
        ...
        case ZodIssueCode.invalid_string:
            typeof issue.validation == "object" ? "includes" in issue.validation ? (message = 'Invalid input: must include "'.concat(issue.validation.includes, '"'), typeof issue.validation.position == "number" && (message = "".concat(message, " at one or more positions greater than or equal to ").concat(issue.validation.position))) : "startsWith" in issue.validation ? message = 'Invalid input: must start with "'.concat(issue.validation.startsWith, '"') : "endsWith" in issue.validation ? message = 'Invalid input: must end with "'.concat(issue.validation.endsWith, '"') : util.assertNever(issue.validation) : issue.validation !== "regex" ? message = "Invalid ".concat(issue.validation) : message = "Invalid";
            break;
        case ZodIssueCode.too_small:
            issue.type === "array" ? message = "Array must contain ".concat(issue.exact ? "exactly" : issue.inclusive ? "at least" : "more than", " ").concat(issue.minimum, " element(s)") : issue.type === "string" ? message = "String must contain ".concat(issue.exact ? "exactly" : issue.inclusive ? "at least" : "over", " ").concat(issue.minimum, " character(s)") : issue.type === "number" ? message = "Number must be ".concat(issue.exact ? "exactly equal to " : issue.inclusive ? "greater than or equal to " : "greater than ").concat(issue.minimum) : issue.type === "bigint" ? message = "Number must be ".concat(issue.exact ? "exactly equal to " : issue.inclusive ? "greater than or equal to " : "greater than ").concat(issue.minimum) : issue.type === "date" ? message = "Date must be ".concat(issue.exact ? "exactly equal to " : issue.inclusive ? "greater than or equal to " : "greater than ").concat(new Date(Number(issue.minimum))) : message = "Invalid input";
            break;
        case ZodIssueCode.too_big:
            issue.type === "array" ? message = "Array must contain ".concat(issue.exact ? "exactly" : issue.inclusive ? "at most" : "less than", " ").concat(issue.maximum, " element(s)") : issue.type === "string" ? message = "String must contain ".concat(issue.exact ? "exactly" : issue.inclusive ? "at most" : "under", " ").concat(issue.maximum, " character(s)") : issue.type === "number" ? message = "Number must be ".concat(issue.exact ? "exactly" : issue.inclusive ? "less than or equal to" : "less than", " ").concat(issue.maximum) : issue.type === "bigint" ? message = "BigInt must be ".concat(issue.exact ? "exactly" : issue.inclusive ? "less than or equal to" : "less than", " ").concat(issue.maximum) : issue.type === "date" ? message = "Date must be ".concat(issue.exact ? "exactly" : issue.inclusive ? "smaller than or equal to" : "smaller than", " ").concat(new Date(Number(issue.maximum))) : message = "Invalid input";
            break;
...
const makeIssue = (params)=>{
    const { data, path, errorMaps, issueData } = params, fullPath = [
        ...path,
        ...issueData.path || []
    ], fullIssue = {
        ...issueData,
...
const INVALID = Object.freeze({
    status: "aborted"
        ...
        status: "valid",
        value
    }), isAborted = (x2)=>x2.status === "aborted", isDirty = (x2)=>x2.status === "dirty", isValid = (x2)=>x2.status === "valid", isAsync = (x2)=>typeof Promise < "u" && x2 instanceof Promise;
var errorUtil;
(function(errorUtil2) {
    errorUtil2.errToObj = (message)=>typeof message == "string" ? {
            message
        } : message || {}, errorUtil2.toString = (message)=>typeof message == "string" ? message : message == null ? void 0 : message.message;
})(errorUtil || (errorUtil = {}));
...
function processCreateParams(params) {
    ...
    return errorMap2 ? {
        ...
        errorMap: (iss, ctx)=>{
            ...
            var _ref, _ref1;
            return iss.code === "invalid_enum_value" ? {
...
class ZodType {
    ...
    safeParse(data, params) {
        ...
        const ctx = {
            common: {
                issues: [],
                async: (_ref = params == null ? void 0 : params.async) !== null && _ref !== void 0 ? _ref : !1,
    ...
    refine(check, message) {
        const getIssueProperties = (val)=>typeof message == "string" || typeof message > "u" ? {
                message
            } : typeof message == "function" ? message(val) : message;
...
function datetimeRegex(args) {
    ...
    const opts = [];
    return opts.push(args.local ? "Z?" : "Z"), args.offset && opts.push("([+-]\\d{2}:?\\d{2})"), regex = "".concat(regex, "(").concat(opts.join("|"), ")"), new RegExp("^".concat(regex, "$"));
}
...
class ZodString extends ZodType {
    _parse(input) {
        ...
        let ctx;
        for (const check of this._def.checks)if (check.kind === "min") input.data.length < check.value && (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
        ...
        else if (check.kind === "max") input.data.length > check.value && (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
        ...
        } else if (check.kind === "email") emailRegex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "email",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
        else if (check.kind === "emoji") emojiRegex || (emojiRegex = new RegExp(_emojiRegex, "u")), emojiRegex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "emoji",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
        else if (check.kind === "uuid") uuidRegex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "uuid",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
        else if (check.kind === "nanoid") nanoidRegex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "nanoid",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
        else if (check.kind === "cuid") cuidRegex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "cuid",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
        else if (check.kind === "cuid2") cuid2Regex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "cuid2",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
        else if (check.kind === "ulid") ulidRegex.test(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            validation: "ulid",
            code: ZodIssueCode.invalid_string,
            message: check.message
        }), status.dirty());
    ...
    datetime(options) {
        ...
        return typeof options == "string" ? this._addCheck({
            ...
            offset: (_ref = options == null ? void 0 : options.offset) !== null && _ref !== void 0 ? _ref : !1,
            local: (_ref1 = options == null ? void 0 : options.local) !== null && _ref1 !== void 0 ? _ref1 : !1,
   ...
   */ nonempty(message) {
    ...
    get minLength() {
        let min = null;
        for (const ch of this._def.checks)ch.kind === "min" && (min === null || ch.value > min) && (min = ch.value);
        return min;
    }
    get maxLength() {
        let max = null;
        for (const ch of this._def.checks)ch.kind === "max" && (max === null || ch.value < max) && (max = ch.value);
        return max;
    }
}
ZodString.create = (params)=>{
    ...
    return new ZodString({
        ...
        coerce: (_ref = params == null ? void 0 : params.coerce) !== null && _ref !== void 0 ? _ref : !1,
...
class ZodNumber extends ZodType {
    _parse(input) {
        ...
        let ctx;
        const status = new ParseStatus();
        for (const check of this._def.checks)check.kind === "int" ? util.isInteger(input.data) || (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            code: ZodIssueCode.invalid_type,
            expected: "integer",
            received: "float",
            message: check.message
...
class ZodBigInt extends ZodType {
    _parse(input) {
        ...
        let ctx;
        const status = new ParseStatus();
        for (const check of this._def.checks)check.kind === "min" ? (check.inclusive ? input.data < check.value : input.data <= check.value) && (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            code: ZodIssueCode.too_small,
            type: "bigint",
            minimum: check.value,
            inclusive: check.inclusive,
...
class ZodDate extends ZodType {
    _parse(input) {
        ...
        const status = new ParseStatus();
        let ctx;
        for (const check of this._def.checks)check.kind === "min" ? input.data.getTime() < check.value && (ctx = this._getOrReturnCtx(input, ctx), addIssueToContext(ctx, {
            code: ZodIssueCode.too_small,
            message: check.message,
            inclusive: !0,
            exact: !1,
...
class ZodArray extends ZodType {
    _parse(input) {
        ...
        }), status.dirty()), ctx.common.async) return Promise.all([
            ...ctx.data
        ].map((item, i2)=>def.type._parseAsync(new ParseInputLazyPath(ctx, item, ctx.path, i2)))).then((result2)=>ParseStatus.mergeArray(status, result2));
        const result = [
            ...ctx.data
        ].map((item, i2)=>def.type._parseSync(new ParseInputLazyPath(ctx, item, ctx.path, i2)));
...
class ZodObject extends ZodType {
    ...
    _parse(input) {
        ...
        if (!(this._def.catchall instanceof ZodNever && this._def.unknownKeys === "strip")) for(const key in ctx.data)shapeKeys.includes(key) || extraKeys.push(key);
        const pairs = [];
        ...
        if (this._def.catchall instanceof ZodNever) {
            ...
            else if (unknownKeys === "strict") extraKeys.length > 0 && (addIssueToContext(ctx, {
        ...
        return errorUtil.errToObj, new ZodObject({
            ...
            ...message !== void 0 ? {
                errorMap: (issue, ctx)=>{
                    var _a, _b;
                    var _ref;
                    const defaultError = (_ref = (_b = (_a = this._def).errorMap) == null ? void 0 : _b.call(_a, issue, ctx).message) !== null && _ref !== void 0 ? _ref : ctx.defaultError;
                    var _errorUtil_errToObj_message;
                    return issue.code === "unrecognized_keys" ? {
                        message: (_errorUtil_errToObj_message = errorUtil.errToObj(message).message) !== null && _errorUtil_errToObj_message !== void 0 ? _errorUtil_errToObj_message : defaultError
                    } : {
   ...
   */ merge(merging) {
    ...
    pick(mask) {
        const shape = {};
        for (const key of util.objectKeys(mask))mask[key] && this.shape[key] && (shape[key] = this.shape[key]);
        return new ZodObject({
            ...this._def,
            shape: ()=>shape
        });
    ...
    omit(mask) {
        const shape = {};
        for (const key of util.objectKeys(this.shape))mask[key] || (shape[key] = this.shape[key]);
        return new ZodObject({
            ...this._def,
            shape: ()=>shape
        });
    ...
    _parse(input) {
        ...
        })).then(handleResults);
        {
            ...
            for (const option of options){
                ...
                result.status === "dirty" && !dirty && (dirty = {
                    result,
                    ctx: childCtx
...
function mergeValues(a2, b2) {
    ...
    if (aType === ZodParsedType.object && bType === ZodParsedType.object) {
        const bKeys = util.objectKeys(b2), sharedKeys = util.objectKeys(a2).filter((key)=>bKeys.indexOf(key) !== -1), newObj = {
            ...a2,
            ...b2
        };
    ...
    } else return aType === ZodParsedType.date && bType === ZodParsedType.date && +a2 == +b2 ? {
        valid: !0,
        data: a2
    } : {
        valid: !1
...
class ZodMap extends ZodType {
    ...
    _parse(input) {
        ...
        const keyType = this._def.keyType, valueType = this._def.valueType, pairs = [
            ...ctx.data.entries()
        ].map((param, index)=>{
            let [key, value] = param;
            return {
...
class ZodPromise extends ZodType {
    ...
    _parse(input) {
        const { ctx } = this._processInputParams(input);
        if (ctx.parsedType !== ZodParsedType.promise && ctx.common.async === !1) return addIssueToContext(ctx, {
            code: ZodIssueCode.invalid_type,
    ...
    _parse(input) {
        ...
        if (effect.type === "refinement") {
            ...
            if (ctx.common.async === !1) {
                ...
                return inner.status === "aborted" ? INVALID : (inner.status === "dirty" && status.dirty(), executeRefinement(inner.value), {
                    status: status.value,
                    value: inner.value
                });
                ...
                path: ctx.path,
                parent: ctx
            }).then((inner)=>inner.status === "aborted" ? INVALID : (inner.status === "dirty" && status.dirty(), executeRefinement(inner.value).then(()=>({
                        status: status.value,
                        value: inner.value
                    }))));
        }
        if (effect.type === "transform") if (ctx.common.async === !1) {
...
var DEFAULT_PORT = 5746, PING_ENDPOINT = "/ping/stagewise", PING_RESPONSE = "21st-extension", EventName = /* @__PURE__ */ ((EventName2)=>(EventName2.EXTENSION_ACTIVATED = "extension_activated", EventName2.ACTIVATION_ERROR = "activation_error", EventName2.OPENED_WEB_APP_WORKSPACE = "opened_web_app_workspace", EventName2.GETTING_STARTED_PANEL_SHOWN = "getting_started_panel_shown", EventName2.GETTING_STARTED_PANEL_MANUAL_SHOW = "getting_started_panel_manual_show", EventName2.INTERACTED_WITH_GETTING_STARTED_PANEL = "interacted_with_getting_started_panel", EventName2.DISMISSED_GETTING_STARTED_PANEL = "dismissed_getting_started_panel", EventName2.CLICKED_SETUP_TOOLBAR_IN_GETTING_STARTED_PANEL = "clicked_setup_toolbar_in_getting_started_panel", EventName2.CLICKED_OPEN_DOCS_IN_GETTING_STARTED_PANEL = "clicked_open_docs_in_getting_started_panel", EventName2.POST_SETUP_FEEDBACK = "post_setup_feedback", EventName2.TOOLBAR_AUTO_SETUP_STARTED = "toolbar_auto_setup_started", EventName2.TOOLBAR_CONNECTED = "toolbar_connected", EventName2.AGENT_PROMPT_TRIGGERED = "agent_prompt_triggered", EventName2.OPEN_EXTERNAL_URL = "open_external_url", EventName2.SHOW_TOOLBAR_UPDATE_NOTIFICATION = "show_toolbar_update_notification", EventName2.TOOLBAR_UPDATE_NOTIFICATION_AUTO_UPDATE = "toolbar_update_notification_auto_update", EventName2.TOOLBAR_UPDATE_NOTIFICATION_IGNORED = "toolbar_update_notification_ignored", EventName2.TOOLBAR_UPDATE_NOTIFICATION_DISMISSED = "toolbar_update_notification_dismissed", EventName2.SHOW_TOOLBAR_INTEGRATION_NOTIFICATION = "show_toolbar_integration_notification", EventName2.TOOLBAR_INTEGRATION_NOTIFICATION_IGNORE = "toolbar_integration_notification_ignore", EventName2.TOOLBAR_INTEGRATION_NOTIFICATION_DISMISSED = "toolbar_integration_notification_dismissed", EventName2.TOOLBAR_AUTO_UPDATE_PROMPT_SENT = "toolbar_auto_update_prompt_sent", EventName2.MAGIC_CHAT_TRIGGERED = "magic_chat_triggered", EventName2.COMPONENTS_SEARCH_TRIGGERED = "components_search_triggered", EventName2.COMPONENT_SELECTED = "component_selected", EventName2.AGENT_PROMPT_TRIGGERED_WITH_COMPONENT = "agent_prompt_triggered_with_component", EventName2))(EventName || {}), EventNameSchema = enumType(Object.values(EventName)), contract = {
    server: {
        ...
        trackEvent: {
            request: objectType({
                sessionId: stringType().optional().describe("Session ID for validation"),
                eventName: EventNameSchema.describe("The analytics event name to track"),
...
function M(n, e2) {
    ...
    function u2(e3) {
        return this.shouldComponentUpdate = r2, (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(n, e3);
    }
    return u2.displayName = "Memo(" + (n.displayName || n.name) + ")", u2.prototype.isReactComponent = !0, u2.__f = !0, u2;
}
(N.prototype = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["x"]()).isPureReactComponent = !0, N.prototype.shouldComponentUpdate = function(n, t2) {
    return E(this.props, n) || E(this.state, t2);
};
var T = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].__b;
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].__b = function(n) {
...
var L = function(n, t2) {
    return n == null ? null : (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])(n).map(t2));
}, O = {
    map: L,
    forEach: L,
    count: function(n) {
        return n ? (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])(n).length : 0;
    },
    only: function(n) {
        var t2 = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])(n);
        if (t2.length !== 1) throw "Children.only";
        return t2[0];
    },
    toArray: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"]
}, F = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].__e;
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].__e = function(n, t2, e2, r2) {
    if (n.then) {
        for(var u2, o2 = t2; o2 = o2.__;)if ((u2 = o2.__c) && u2.__c) return t2.__e == null && (t2.__e = e2.__e, t2.__k = e2.__k), u2.__c(n, t2);
    }
    F(n, t2, e2, r2);
};
var U = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].unmount;
function V(n, t2, e2) {
...
function z(n) {
    ...
    function o2(o3) {
        ...
        if (!r2) throw e2;
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(r2, o3);
    }
...
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].unmount = function(n) {
    var t2 = n.__c;
    t2 && t2.__R && t2.__R(), t2 && 32 & n.__u && (n.type = null), U && U(n);
}, (P.prototype = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["x"]()).__c = function(n, t2) {
    ...
    var i2 = e2.__a && (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["k"], null, n.fallback);
    return i2 && (i2.__u &= -33), [
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["k"], null, e2.__a ? null : n.children),
        i2
    ];
};
...
function Y(n) {
    ...
    if (e2.componentWillUnmount = function() {
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["E"])(null, e2.v), e2.v = null, e2.h = null;
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["E"])((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(Z, {
        context: e2.context
    }, n.__v), e2.v);
}
function $(n, e2) {
    var r2 = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(Y, {
        __v: n,
        h: e2
    });
    return r2.containerInfo = e2, r2;
}
(B.prototype = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["x"]()).__a = function(n) {
    ...
    var t2 = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])(n.children);
...
function nn(n, t2, e2) {
    return t2.__k == null && (t2.textContent = ""), (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["E"])(n, t2), typeof e2 == "function" && e2(), n ? n.__c : null;
}
function tn(n, t2, e2) {
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["G"])(n, t2), typeof e2 == "function" && e2(), n ? n.__c : null;
}
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["x"].prototype.isReactComponent = {}, [
    "componentWillMount",
    "componentWillReceiveProps",
    "componentWillUpdate"
].forEach(function(t2) {
    Object.defineProperty(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["x"].prototype, t2, {
        configurable: !0,
        get: function() {
...
var en = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].event;
function rn() {}
function un() {
...
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].event = function(n) {
...
}, fn = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].vnode;
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].vnode = function(n) {
    typeof n.type == "string" && function(n2) {
        ...
        e2 == "select" && u2.multiple && Array.isArray(u2.value) && (u2.value = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])(t2.children).forEach(function(n3) {
            n3.props.selected = u2.value.indexOf(n3.props.value) != -1;
        })), e2 == "select" && u2.defaultValue != null && (u2.value = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["H"])(t2.children).forEach(function(n3) {
...
var an = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].__r;
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].__r = function(n) {
    an && an(n), ln = n.__c;
};
var sn = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].diffed;
__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["l"].diffed = function(n) {
    sn && sn(n);
...
function dn(n) {
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"].bind(null, n);
}
function mn(n) {
    return !!n && n.$$typeof === q;
}
function pn(n) {
    return mn(n) && n.type === __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["k"];
}
function yn(n) {
    return !!n && !!n.displayName && (typeof n.displayName == "string" || n.displayName instanceof String) && n.displayName.startsWith("Memo(");
}
function _n(n) {
    return mn(n) ? __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["J"].apply(null, arguments) : n;
}
function bn(n) {
    return !!n.__k && ((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["E"])(null, n), !0);
}
function Sn(n) {
...
var gn = function(n, t2) {
    ...
    return n(t2);
}, Cn = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["k"], xn = mn, Rn = {
    useState: d,
    useId: g$1,
    useReducer: h,
    useEffect: y,
    ...
    createPortal: $,
    createElement: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"],
    createContext: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["K"],
    createFactory: dn,
    cloneElement: _n,
    createRef: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["b"],
    Fragment: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["k"],
    isValidElement: mn,
    isElement: xn,
    isFragment: pn,
    isMemo: yn,
    findDOMNode: Sn,
    Component: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["x"],
    PureComponent: N,
    memo: M,
    forwardRef: D,
    flushSync: En,
...
async function discoverVSCodeWindows() {
    let maxAttempts = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 10, timeout = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 300;
    const windows = [];
    let consecutiveErrors = 0;
    for(let attempt = 0; attempt < maxAttempts; attempt++){
        const port = DEFAULT_PORT + attempt;
        try {
            const controller = new AbortController(), timeoutId = setTimeout(()=>controller.abort(), timeout), response = await fetch("http://localhost:".concat(port).concat(PING_ENDPOINT), {
                signal: controller.signal
            });
            if (clearTimeout(timeoutId), consecutiveErrors = 0, response.ok && await response.text() === PING_RESPONSE) try {
                const bridge = createSRPCClientBridge("ws://localhost:".concat(port), contract);
                ...
                console.warn("Failed to get session info from port ".concat(port, ":"), error);
            }
            else continue;
        } catch (e) {
            if (consecutiveErrors++, consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                console.warn("⬆️⬆️⬆️ Those two errors are expected! (Everything is fine, they are part of stagewise's discovery mechanism!) ✅");
                break;
            }
            continue;
        }
    }
    return windows.length === 0 && console.warn("No IDE windows found, please start an IDE with the stagewise extension installed! ❌"), windows;
}
const getCurrentPort = ()=>typeof window < "u" && window.location && window.location.port || "80", getStorageKey = ()=>"ide-selected-session-id-on-browser-port-".concat(getCurrentPort()), getStoredSessionId = ()=>{
    try {
...
}, VSCodeContext = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["K"])({
    windows: [],
    isDiscovering: !1,
    discoveryError: null,
...
function VSCodeProvider(param) {
    ...
    const selectedSession = windows.find((w2)=>w2.sessionId === selectedSessionId), value = {
        windows,
        isDiscovering,
        discoveryError,
        selectedSession,
    ...
    return /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["u"])(VSCodeContext.Provider, {
        value,
        children
    });
}
...
const SRPCBridgeContext = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["K"])({
    bridge: null,
    isConnecting: !1,
    error: null
});
function SRPCBridgeProvider(param) {
    let { children } = param;
    ...
    return y(()=>{
        ...
        selectedSession,
        initializeBridge
    ]), /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["u"])(SRPCBridgeContext.Provider, {
        value: state,
        children
    });
}
function useSRPCBridge() {
    ...
    if (!context) throw new Error("useSRPCBridge must be used within an SRPCBridgeProvider");
    return context;
}
const PluginContext = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["K"])({
    plugins: [],
    toolbarContext: {
        sendPrompt: ()=>{}
    }
});
function PluginProvider(param) {
    let { children } = param;
    const { bridge } = useSRPCBridge(), { selectedSession } = useVSCode(), { config } = useConfig(), plugins = (config == null ? void 0 : config.plugins) || [], toolbarContext = T$1(()=>({
            sendPrompt: async (prompt)=>{
                ...
                return await bridge.call.triggerAgentPrompt(typeof prompt == "string" ? {
                    prompt,
                    ...selectedSession && {
    ...
    return /* @__PURE__ */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$21st$2d$extension$2f$toolbar$2f$dist$2f$jsx$2d$runtime$2d$BI3iOU20$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["u"])(PluginContext.Provider, {
        value,
        children
    });
}
...
const CLASS_PART_SEPARATOR = "-", createClassGroupUtils = (config)=>{
    const classMap = createClassMap(config), { conflictingClassGroups, conflictingClassGroupModifiers } = config;
    return {
    ...
    return (_a = classPartObject.validators.find((param)=>{
        let { validator } = param;
    ...
    const { theme, classGroups } = config, classMap = {
    ...
    return {
        get (key) {
            ...
            if ((value = previousCache.get(key)) !== void 0) return update(key, value), value;
        },
        set (key, value) {
    ...
    const { parseClassName, getClassGroupId, getConflictingClassGroupIds, sortModifiers } = configUtils, classGroupsInConflict = [], classNames = classList.trim().split(SPLIT_CLASSES_REGEX);
    let result = "";
    for(let index = classNames.length - 1; index >= 0; index -= 1){
        const originalClassName = classNames[index], { isExternal, modifiers, hasImportantModifier, baseClassName, maybePostfixModifierPosition } = parseClassName(originalClassName);
        if (isExternal) {
...
function twJoin() {
    ...
    for(; index < arguments.length;)(argument = arguments[index++]) && (resolvedValue = toValue(argument)) && (string && (string += " "), string += resolvedValue);
    return string;
}
const toValue = (mix)=>{
    if (typeof mix == "string") return mix;
    let resolvedValue, string = "";
    for(let k2 = 0; k2 < mix.length; k2++)mix[k2] && (resolvedValue = toValue(mix[k2])) && (string && (string += " "), string += resolvedValue);
    return string;
};
...
}, extendTailwindMerge = function(configExtension) {
    for(var _len = arguments.length, createConfig = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++){
        createConfig[_key - 1] = arguments[_key];
    }
    return typeof configExtension == "function" ? createTailwindMerge(getDefaultConfig, configExtension, ...createConfig) : createTailwindMerge(()=>mergeConfigs(getDefaultConfig(), configExtension), ...createConfig);
}, companionAnchorTagName = "stagewise-companion-anchor";
function getElementAtPoint(x2, y2) {
    return document.elementsFromPoint(x2, y2).find((element)=>element.nodeName !== "STAGEWISE-COMPANION-ANCHOR" && !element.closest(companionAnchorTagName) && !element.closest("svg") && isElementAtPoint(element, x2, y2)) || document.body;
}
...
async function fetchSVGContent(url) {
    ...
    const attempts = [
        ...
        ()=>fetch(url),
        // CORS proxies
        ()=>fetch("https://corsproxy.io/?".concat(url)),
        ()=>fetch("https://cors-anywhere.herokuapp.com/".concat(url)),
        ()=>fetch("https://api.allorigins.win/get?url=".concat(encodeURIComponent(url)))
    ];
    for(let i2 = 0; i2 < attempts.length; i2++)try {
        const response = await attempts[i2]();
        if (!response.ok) throw new Error("HTTP ".concat(response.status));
        let svgContent;
        if (i2 === 3 ? svgContent = (await response.json()).contents : svgContent = await response.text(), !svgContent.trim().startsWith("<svg")) throw new Error("Response is not valid SVG");
        return svgContent = svgContent.trim(), svgContent = svgContent.replace(/^<\?xml[^>]*>\s*/, ""), svgCache.set(url, svgContent), svgContent;
    } catch (err) {
        if (console.warn("SVG fetch attempt ".concat(i2 + 1, " failed:"), err), i2 === attempts.length - 1) return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>';
    }
...
function processSVGForInline(svgContent, className) {
    ...
    return processedSvg.includes("class=") ? processedSvg = processedSvg.replace(/class="([^"]*)"/, 'class="$1 '.concat(finalClassName, '"')) : processedSvg = processedSvg.replace(/<svg([^>]*)>/, '<svg$1 class="'.concat(finalClassName, '">')), processedSvg;
}
;
}),
}]);

//# sourceMappingURL=node_modules_%4021st-extension_toolbar_dist_utils-DRt8LwD0_8759e975.js.map