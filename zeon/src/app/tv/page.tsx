...
import { getImageUrl, TV } from "../../lib/getImageUrl";
...
export default function TVPage() {
  ...
  const [watchLater, setWatchLater] = useState<TV[]>([]);

  useEffect(() => {
    setLoading(true);
    fetch(`${TMDB_BASE}/tv/popular?api_key=${TMDB_API_KEY}&page=${page}`)
      .then(res => res.json())
      .then(data => {
  ...
  const isInWatchLater = (item: TV) => watchLater.some((w) => w.id === item.id);

  const goToPage = (p: number) => {
    router.push(`/tv?page=${p}`);
  };

  return (
    <div className="max-w-7xl mx-auto px-4 py-8 mt-16">
      <h1 className="text-3xl font-extrabold text-white mb-8 drop-shadow-lg">Popular TV Shows</h1>
      {loading ? (
            ...
            >
              ...
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  if (isInWatchLater(show)) {
                    setWatchLater((prev) => prev.filter((w) => w.id !== show.id));
                  } else {
                    setWatchLater((prev) => [...prev, show]);
                  }
                }}
                aria-label={isInWatchLater(show) ? "Remove from Watch Later" : "Add to Watch Later"}
                tabIndex={0}
                type="button"
              >
                <IconBookmark size={22} stroke={2.2} className={isInWatchLater(show) ? "text-purple-500 fill-purple-500" : "text-white/80"} />
              </button>
              {/* Poster */}
              <Link href={`/watch/tv/${show.id}`} className="block focus:outline-none focus:ring-2 focus:ring-cyan-400 min-w-[180px] max-w-[180px] rounded-2xl overflow-hidden" tabIndex={0} aria-label={`Open ${show.name}`}>
              ...
              >
                <div className="font-bold text-lg text-white mb-1 truncate" title={show.name}>{show.name}</div>
                <div className="text-white/80 text-xs mb-1 line-clamp-2">{show.overview}</div>
              </motion.div>
            </motion.div>
          ))}
        </div>
